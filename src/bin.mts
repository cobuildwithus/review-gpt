...
const cli = Cli.create('cobuild-review-gpt', {
  ...
  options: z.object({
    ...
    responseMarker: z.string().optional().describe('Only treat a captured response as final when it contains this exact text; marked concrete-model reviews that complete in under 7.5m fail as untrusted (use with --wait).'),
