...
const cli = Cli.create('cobuild-review-gpt', {
  ...
  options: z.object({
    ...
    responseMarker: z.string().optional().describe('Only treat a captured response as final when it contains this exact text; marked concrete-model reviews shorter than --minimum-marked-response-time fail as untrusted (use with --wait).'),
