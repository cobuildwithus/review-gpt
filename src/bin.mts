...
const cli = Cli.create('cobuild-review-gpt', {
  ...
  options: z.object({
    ...
    model: z.string().optional().describe('Draft model target. gpt-5.6-sol (default) and pro target the current ChatGPT Pro model. Waited concrete-model sends require an explicit matching MODEL_CONFIRMATION response line.'),
    ...
    responseMarker: z.string().optional().describe('Only treat a captured response as final when it contains this exact text; marked concrete-model reviews that complete in under 10m fail as untrusted (use with --wait).'),
