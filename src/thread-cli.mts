...
export function createThreadCli() {
  ...
  cli.command('wake', {
    ...
    output: z.object({
      ...
      childSessionPersistence: z.enum(['pending', 'verified']).optional().describe('Whether the spawned child session already had a canonical ID-bearing rollout filename in the resolved Codex home at handoff time.'),
      ...
      recursive: z.object({
        descendantOutputDir: z.string().describe('Deterministic output directory for the next recursive wake hop.'),
        descendantStatusPath: z.string().describe('Expected status.json path for the next recursive wake hop.'),
        descendantWakeLaunchPath: z.string().describe('Path where the generated recursive helper stores the detached wake launch JSON.'),
        descendantWakeLogPath: z.string().describe('Path where the generated recursive helper stores the detached wake stderr log.'),
      ...
      }).optional().describe('Generated recursive helper paths and deterministic descendant wake metadata, when --recursive-depth is enabled.'),
