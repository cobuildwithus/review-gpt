...
- Waited runs that request a concrete model require the response's explicit `MODEL_CONFIRMATION: <model>` line and fail closed on an omitted, `UNKNOWN`, or different model. When ChatGPT model metadata is available, capture also validates it, including the UI-only GPT-5.6 Sol name's `gpt-5-6-pro` backend slug; both signals must agree.
