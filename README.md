...
- Browsers throttle background-tab timers, which can freeze the DOM the capture reads mid-stream. Balanced managed-browser mode keeps those timers reliable while allowing normal renderer and occluded-window scheduling; the capture session pins focus emulation and an active page lifecycle only for its owned page over CDP, so polling works in the background without forcing every lane tab to render at foreground priority or stealing OS focus.
