...
- Waited runs that request a concrete model validate the response's ChatGPT model metadata first, including the UI-only GPT-5.6 Sol name's `gpt-5-6-pro` backend slug. When that metadata is unavailable, capture falls back to the prompt's `MODEL_CONFIRMATION: <model>` line and fails closed on an omitted, `UNKNOWN`, or different model; matching response metadata remains authoritative even when the assistant cannot identify itself in prose.
