...
- `thread wake` reuses an existing matching thread tab when possible, can reuse a generic `https://chatgpt.com/` home tab instead of spawning another blank tab, and treats `/c/<thread-id>` URLs with extra query parameters as the same thread.
